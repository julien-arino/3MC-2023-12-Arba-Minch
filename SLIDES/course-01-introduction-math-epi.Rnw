...
\begin{frame}
A point $x^\star\in W$ is an \emph{equilibrium} if $f(x^\star)=0$
\vfill
\begin{definition}[Locally stable equilibrium]\label{def:LS_EP}
An equilibrium point $x^\star$ of \eqref{eq:ODE} is \emph{locally stable} if for every neighbourhood $\mathcal{N}(x^\star)$ of $x^\star$ in $W$, there is a neighbourhood $\mathcal{N}_1\subseteq\mathcal{N}(x^\star)$ of $x^\star$ such that every solution $x(t,x_0)$ with $x_0\in\mathcal{N}_1$ is defined and in $\mathcal{N}(x^\star)$ for all $t>t_0$
\end{definition}
\vfill
\begin{definition}[Locally asymptotically stable equilibrium]
If $\mathcal{N}_1$ can be chosen so that in addition to the properties in Definition~\ref{def:LS_EP}, $\lim_{t\to\infty}x(t,x_0)=x^\star$ for all $x_0\in\mathcal{N}_1$, then $x^\star$ is locally asymptotically stable
\end{definition}
\end{frame}

\begin{frame}
The problem with the DFE \eqref{eq:DFE_KMK} of \eqref{sys:KMK} is that they are not \emph{isolated}: any (open) neighbourhood of an equilibrium contains infinitely many other equilibria; see Figure~\ref{fig:neighbourhood_KMK}
\vfill
The consequence of this is that stability cannot be asymptotic
\end{frame}
...
\begin{figure}
    ...
    \end{tikzpicture}    
    \caption{Ball: part of a neighbourhood of an equilibrium point $x^\star\in E_0^\text{KMK}$ lying on the $S-R$ plane (the neighbourhood also extends above and below the $S-R$ plane in the $I$ direction). 
    The thin line is $E_0^\text{KMK}$, while the thick line is the intersection of $E_0^\text{KMK}$ with the neighbourhood of $x^\star$.}
    \label{fig:neighbourhood_KMK}
\end{figure}
\end{frame}

\begin{frame}
\begin{proposition}
Consider a disease-free equilibrium $x^\star\in E_0^\text{KMK}$ of \eqref{sys:KMK}. Then $x^\star$ is locally stable but not locally asymptotically stable  
\end{proposition}
\end{frame}
...
Therefore, any open neighbourhood of $x_1^\star$ contains points $x_0$ not such that $\lim_{t\to\infty}x(t,x_0)=x_1^\star$ and it follows that $x_1^\star$ is locally stable but not locally asymptotically stable.
\end{frame}

\begin{frame}
It is important to make a remark here concerning the use of \emph{next generation matrix} methods in this context.
Consider, e.g., the method in \cite{VdDWatmough2002}.
There, one assumption (namely, assumption A5) is that the disease-free equilibrium be \emph{locally asymptotically stable}, with the assumption that all eigenvalues of the linearisation near a disease-free equilibrium have negative real parts.
\end{frame}
